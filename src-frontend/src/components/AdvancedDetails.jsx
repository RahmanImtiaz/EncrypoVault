...
const AdvancedDetails = ({ cryptoId }) => {
    ...
    return (
        <div>
            ...
            <div className="technical-details">
                ...
                <p>Block Time (seconds): {cryptoData.block_time_in_minutes ? cryptoData.block_time_in_minutes * 60 : "N/A"}</p>

                <div className="categories">
                    <p>Categories: {cryptoData.categories ? cryptoData.categories.join(", ") : "N/A"}</p>
                </div>
            ...
            <div className="ath-data">
                <p> ATH Price: {getValue(cryptoData.market_data?.ath?.gbp)}</p>

                <p> ATH Change Percentage: {getValue(cryptoData.market_data?.ath_change_percentage?.gbp)}</p>
                
                <p> ATH Date: {getValue(cryptoData.market_data?.ath_date?.gbp)}</p>

            </div>

            <div className="atl-data">
                <p> ATL Price: {getValue(cryptoData.market_data?.atl?.gbp)}</p>

                <p> ATL Change Percentage: {getValue(cryptoData.market_data?.atl_change_percentage?.gbp)}</p>
                
                <p> ATL Date: {getValue(cryptoData.market_data?.atl_date?.gbp)}</p>
            </div>

            <div className="market-data">
                <p>Price Change: {getValue(cryptoData.market_data?.price_change_24h_in_currency?.gbp)}</p>
                
                <p> Fully Diluted Valuation: {(cryptoData.market_data?.fully_diluted_valuation?.gbp)}</p>

                <p> Market Cap: {getValue(cryptoData.market_data?.market_cap?.gbp)}</p>

                <p> Market Cap Change: {getValue(cryptoData.market_data?.market_cap_change_24h_in_currency?.gbp)}</p>

                <p> Market Cap Change Percentage: {getValue(cryptoData.market_data?.market_cap_change_percentage_24h)}</p>


            </div>

            <div className="supply-metrics">
                <p> Circulating Supply: {getValue(cryptoData.market_data?.circulating_supply)}</p>

                <p> Total Supply: {getValue(cryptoData.market_data?.total_supply)}</p>

                <p> Max Supply: {getValue(cryptoData.market_data?.max_supply)}</p>

                <p> Total Volume: {getValue(cryptoData.market_data?.total_volume?.gbp)}</p>
            </div>
